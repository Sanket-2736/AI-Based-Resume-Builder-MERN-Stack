...
const SkillsForm = ({data, onChange}) => {
    ...
    const removeSkill = (index) => {
        onChange(data.filter((_, ind) => ind !== index));
    }
  ...
  return (
    <div className='space-y-4'>
        ...
        <div className='gap-2 flex'>
            <input type='text'
            placeholder='Enter a skill (e.g., JavaScript, Project Management, C++)'            
            className='flex-1 px-3 py-2 text-sm'
            onChange={(e) => setNewSkill(e.target.value)}
            onKeyDown={handleKeyPress}
            />

            <button onClick={addSkill} disabled={!newSkill.trim} className='flex items-center gap-2 px-4 py-2 text-sm
            bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50
        ...
        <div className='bg-blue-50 p-3 rounded-lg'>
            <p className='text-sm text-blue-800'>
                <strong>Tip: </strong> Add 8-12 relevant skills. Include both technical (programming languages, tools) as well as soft skills (leadership, communication).
            </p>
        </div>
    </div>
  )
